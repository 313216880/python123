...
Description:   网站爬虫  https://www.ittime.com.cn/news/zixun.shtml
@author: LiuHao
...
# 详情url
detailsUrlRe=re.compile(r'<h2><a href="(.*?)" target="_blank">.*?</a></h2>')
# 所有的简介
...
detailsUrls=detailsUrlRe.findall(webSourceCode)
...
for detailsUrl in detailsUrls:
    print(detailsUrl)
    print("详情==============================================================")
    webSourceCode=urllib.request.urlopen("https://www.ittime.com.cn"+detailsUrl).read().decode("utf-8","ignore")
    titleRe=re.compile(r'<h2>(.*?)</h2>')
    title=titleRe.findall(webSourceCode)
    print("标题:" + title[0])
    authorRe=re.compile(r'<a target="_blank" href="#">(.*?)</a>')
    author=authorRe.findall(webSourceCode)
    print("作者:" + author[0])
    timeRe=re.compile(r'<span class="time">(.*?)</span>')
    time=timeRe.findall(webSourceCode)
    print("时间:" + time[1])
    contentRe=re.compile(r'<p>(.*?)</p>')
